...
frst_usr_ans, scnd_usr_ans = ask_input.check_input() # scnd_usr_ans can be '1' or '2'
frst_usr_ans, sign = partitioner.negative_sign_partitioner(frst_usr_ans)
...
if scnd_usr_ans == '1': # User wants to transform from binary to decimal
    ...
    dec_num = decimal.Decimal(sign+str(int_dec)+str(float_dec)[1:])
    ...
    bin_num = decimal.Decimal(sign+str(int_bin)+str(float_bin)[1:])
